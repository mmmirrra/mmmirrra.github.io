...
void main() {
	...
	Y1.age = 35;
	Y1.sex = 'M';
	printf("X1: %s, %d, %c\n", X1.name, X1.age, X1.sex);			// Output Results --> X1 : HONG, 30, M:
	printf("Y1: %s, %d, %c\n", Y1.name, Y1.age, Y1.sex);			// Output Results --> Y1 : LIM, 35, M
	printf("sizeof(struct person1) : %d\n", sizeof(struct person1));	// Output Results --> sizeof(struct person1) : 16
	printf("sizeof(X1) : %d\n", sizeof(X1));				// Output Results --> sizeof(X1) : 16
	printf("sizeof(Y1.name) : %d\n", sizeof(Y1.name));			// Output Results --> sizeof(Y1.name) : 8
	printf("sizeof(Y1.age) : %d\n", sizeof(Y1.age));			// Output Results --> sizeof(Y1.age) : 4
	printf("sizeof(Y1.sex) : %d\n", sizeof(Y1.sex));			// Output Results --> sizeof(Y1.sex) : 1
	/* sex에는 person1의 멤버 중 가장 큰 자료형인 int의 크기인 4바이트가 배정됨 */
	printf("sizeof(Y1) : %d\n", sizeof(Y1));				// Output Results --> sizeof(Y1) : 16
	...
	int i2, sum2 = 0;
	for (i2 = 0; i2 < 3; i2++) {
		printf("name:%s age:%d sex:%c\n", X2[i2].name, X2[i2].age, X2[i2].sex);		// Output Results --> name:HONG age:20 sex:M \n name:HWANG age:22 sex:F \n name:LIM age:30 sex:M
		sum2 = sum2 + X2[i2].age;
	}
	printf("age sum:%d\n", sum2);				// Output Results --> age sum:72
	...
	struct student4* p4;
	p4 = hs4;
	printf("%s %d %d\n", p4->name, p4->kor, p4->math);	// Output Results --> KIM HG 90 95
	p4 += 3;
	printf("%s %d %d\n", p4->name, p4->kor, p4->math);	// Output Results --> CHOI HI 95 75
	p4--;
	printf("%s %d %d\n", p4->name, p4->kor, p4->math);	// Output Results --> PARK GS 70 85
	...
	printf("x:%d, y:%d, sum:%d, mul:%d\n", number4.x, number4.y, number4.sum, number4.mul);		// Output Results --> x:10, y:20, sum:30, mul:200
	...
	number5.y = 20;
	calc5(&number5);
	printf("x:%d, y:%d, sum:%d, mul:%d\n", number5.x, number5.y, number5.sum, number5.mul);		// Output Results --> x:10, y:20, sum:30, mul:200

	/* typedef 예제 1 - 2. data6형 구조체 선언과 초기화 */
	DATA6 d6 = { 1, 2 };
	printf("d6.x=%d, d6.y=%d\n", d6.x, d6.y);	// Output Results --> d6.x=1, d6.y=2
	...
	printf("v9.a=%d, v9.b=%d, v9.c=%d, v9.d=%d\n", v9.a, v9.b, v9.c, v9.d);	// Output Results --> v9.a=1, v9.b=2, v9.c=3, v9.d=4
	printf("v9 uses %dbyte.\n", sizeof(v9));				// Output Results --> v9 uses 4byte.
	...
	union test10 u10;
	u10.i = u10.f = u10.d = 0;
	printf("sizeof(u10): %dbyte\n", sizeof(u10));		// Output Results --> sizeof(u10):8byte
	u10.i = 100;
	printf("i:%d f:%f d:%f\n", u10.i, u10.f, u10.d);	// Output Results --> i:100 f:0.000000 d:0.000000
	u10.f = 0.5;
	printf("i:%d f:%f d:%f\n", u10.i, u10.f, u10.d);	// Output Results --> i:0 f:0.500000 d:0.000000
	u10.d = 0.016667;
	printf("i:%d f:%f d:%f\n", u10.i, u10.f, u10.d);	// Output Results --> i:-28156 f:109291767575372798867298844672.000000 d:0.016667
}
