...
// 해당 객체에 커서를 놓고 키를 누르면 필요한 특정 클래스 Import 구문 자동 생성   
...
// 패키지 탐색기 표시   
`Alt + Shift + Q + P` : Java Package Explorer   
// 콘솔 표시   
...
`Ctrl + H` : Find and Replace : Open Search Dialog : Search all files, across projects   
...
// 변수나 메소드 선언부로 이동   
`Ctrl + Click or F3` : Open Structure   
// 해당 메서드나 클래스가 정의된 곳으로 이동   
`F3` : Open Declaration   
// 특정 줄 번호로 이동   
`Ctrl + L` : Go to Line   
// 마지막 편집 위치로 이동   
`Ctrl + Q` : Last Edit Location   
// 에디터 안에서 열린 파일 목록을 미니창으로 확인 후 원하는 파일로 이동   
`Ctrl + E` : Quick Switch Editor   
// 에디터 안에서 열린 파일 목록을 큰 창으로 확인 후 원하는 파일로 이동   
`Ctrl + Shift + E` : Switch to Editor   
// 에디터 안에 열린 파일간 이동 : F6을 누를 때마다 하나씩 순차적으로 이동   
`Ctrl + F6` : Next Editor : Move one by one each time F6 is pressed   
// 이전/다음에 열린 편집기 Edit로 이동   
`Ctrl + PageUp/PageDown` : Next Editor   
// 컴파일 중 에러 등 으로 포커스가 다른데로 갔을 때 Editor 창으로 커서 이동   
`F12` : Activate Editor   
...
`Ctrl + Alt + B` : Enable debug mode for code reload, skipped all breakpoints   
// 현재 라인까지 실행   
`Ctrl + R` : Run to Line   
// 현재 선택한 클래스를 Java 응용 프로그램으로 실행   
